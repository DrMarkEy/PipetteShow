...
import { findPlate } from "./PlatesLib.js";
...
function importXLS(data, fname) {
  ...
  return {
    plate: plate,
    file: {
      values: fillValues(values, plate.wells),
      name: fname,
      substances: headers,
    }
  };
...
function importDivaJSON(contentString, fname) {
  ...
  if (data.info.divaSettings === undefined) {
    alert("Cannot import experiment. Is it a valid DIVA-File?");
    return;
  }
  ...
  let wellNameToId = function (wellName) {
    let letter = wellName.substr(0, 1);
    let number = wellName.substr(1);

    return (
      letter.charCodeAt(0) - "A".charCodeAt(0) + number * data.targetPlate.cols
    );
  };
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    let targetWell = wellNameToId(step.targetWell);
...
function stringFileUploaded($store, fname, content, target) {  
  ...
  try {
    if (fname.endsWith(".pip")) {
      ...
      let results = importDivaJSON(content, fname);
